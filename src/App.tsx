...
function App() {
    const [activeJobId, setActiveJobId] = useState(resumeData.experience[resumeData.experience.length - 1].id);
    ...
    return (
      ...
      `}</style>
            ...
            <section className="max-w-6xl mx-auto px-6 py-16">
                <h2 className="text-3xl font-bold text-gray-900 mb-12 text-center">Experience Timeline</h2>
            ...
            <section className="bg-white border-t border-gray-200">
                <div className="max-w-6xl mx-auto px-6 py-16">
                    ...
                    <div className="grid md:grid-cols-2 gap-8">
                        <div>
                            ...
                            <div className="flex flex-wrap gap-2">
                                {resumeData.skills.programmingLanguages.map((skill, idx) => (
                                    <Badge key={idx} text={skill.name} category="programmingLanguages" logo={skill.logo}
                                           url={skill.url}/>
                                ))}
                            </div>
                        </div>
                        ...
                        <div>
                            <h3 className="text-xl font-semibold text-gray-800 mb-4">Frameworks & Libraries</h3>
                            <div className="flex flex-wrap gap-2">
                                {resumeData.skills.frameworks.map((skill, idx) => (
                                    <Badge key={idx} text={skill.name} category="frameworks" logo={skill.logo}
                                           url={skill.url}/>
                                ))}
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div className="flex flex-wrap gap-2">
                                {resumeData.skills.tools.map((skill, idx) => (
                                    <Badge key={idx} text={skill.name} category="tools" logo={skill.logo}
                                           url={skill.url}/>
                                ))}
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div className="flex flex-wrap gap-2">
                                {resumeData.skills.spokenLanguages.map((skill, idx) => (
                                    <Badge key={idx} text={skill.name} category="spokenLanguages" flag={skill.flag}/>
                                ))}
                            </div>
                        </div>
                    </div>
            ...
            <section className="max-w-6xl mx-auto px-6 py-16">
                ...
                <div className="bg-white rounded-lg p-8 shadow-sm border border-gray-100 max-w-3xl mx-auto">
                    <h3 className="text-xl font-bold text-gray-900">{resumeData.education.degree}</h3>
                    <p className="text-lg text-blue-600 mt-1">{resumeData.education.school}</p>
                    <p className="text-sm text-gray-500 mt-1">{resumeData.education.location} • {resumeData.education.dates}</p>
                    <p className="text-gray-700 mt-4 italic">{resumeData.education.thesis}</p>
                </div>
            </section>
            ...
            <footer className="bg-gray-900 text-gray-400 py-8">
                <div className="max-w-6xl mx-auto px-6 text-center">
                    <p>© {new Date().getFullYear()} {resumeData.name}. Built with React, TypeScript & Tailwind CSS.</p>
                </div>
            </footer>
        </div>
    );
